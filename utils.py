...
class UniverseUtils:
        ...
        # 寻路模式
            ...
            for i in range(30):
                ...
                if time.time() - loop_start_time > 5:
                    if last_recorded_distance is None:
                        last_recorded_distance = now_distance
                    else:
                        if now_distance >= last_recorded_distance - 1:  # 距离没减少至少1像素
                            CUS_LOGGER.warning("长时间未靠近目标，强制后退")
                            key_mouse_manager.keyUp("w")
                            key_mouse_manager.press("s", 0.8)   # 后退
                            if random.choice([True, False]):
                                key_mouse_manager.press("a", 0.6)       # 左移
                                CUS_LOGGER.debug("强制后退后左移")
                            else:
                                key_mouse_manager.press("d", 0.6)       # 右移
                                CUS_LOGGER.debug("强制后退后右移")
                            key_mouse_manager.keyDown("w")
                            # 重置计时器和记录距离
                            loop_start_time = time.time()
                            last_recorded_distance = None
    ...
    def get_shop_only_minimap(self):
        ...
        CUS_LOGGER.debug(f"初始 enter_time = {enter_time}")
        first = self.first_mini
        ...
        init_time = time.time()
        while True:
            CUS_LOGGER.debug("进入交易区域主循环")   # 原 print 替换为 logging

            # 超时脱困逻辑（20秒无交互）
            elapsed = time.time() - enter_time
            CUS_LOGGER.debug(f"循环开始 elapsed = {elapsed:.2f} 秒")
            if elapsed > 20:
                CUS_LOGGER.warning(f"交易区域超过20秒未完成交互 (已过{elapsed:.1f}秒)，强制后退+侧移")
                key_mouse_manager.keyUp("w")
                key_mouse_manager.press("s", 0.8)        # 后退0.8秒
                # 随机左移或右移0.6秒
                if random.choice([True, False]):
        ...
        if need_confirm or self.has_target:
            ...
            for _ in range(max_attempts):
                ...
                # 4. 正常寻路：优先使用 move_to_end（面向觐见装置/出口）
                self.get_screen()
                move_direction = None
                # 尝试通过终点标记（觐见装置）定位
